...
func loudAndRich(richer [][]int, quiet []int) []int {
	res := make([]int, 0)
	richMap := make(map[int][]int)
	for _, rich := range richer {
		if _, ok := richMap[rich[1]]; ok {
			richMap[rich[1]] = append(richMap[rich[1]], rich[0])
		} else {
			richMap[rich[1]] = []int{rich[0]}
		}
	}
	...
	for i := 0; i < len(quiet); i++ {
		smallestPerson := i
		queue.Enqueue(i)
		flag := make([]bool, len(quiet))
		flag[i] = true
		for !queue.IsEmpty() {
			val, _ := queue.Dequeue()
			person := val.(int)
			if quiet[person] < quiet[smallestPerson] {
				smallestPerson = person
			}
			if riches, ok := richMap[person]; ok {
